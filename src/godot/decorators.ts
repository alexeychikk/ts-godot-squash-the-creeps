...
/** Set the script icon */
export function icon<T extends godot.Object>(iconPath: string) {
	return function (target: new() => T) {
		godot.set_script_icon(target, iconPath);
	}
}
...
export function onready<T extends godot.Node>(path: string | (new()=>godot.Node)) {
	return function (target: T, property: string, descriptor?: any) {
		...
		descriptor = descriptor || {};
		descriptor.set = function(v: unknown) { this[key] = v; };
		descriptor.get = function() {
			let v = this[key];
			if (!v) {
				v = (this as godot.Node).get_node(path as string);
				this[key] = v;
			}
			return v;
		};
